...
#
#This sample program will start DATAQ Instrument (11xx/2108/4108/4208/4718) 
# usb data acquisition products
#Please make sure the device is in CDC mode (blinking yellow when conntected)
#Press key 'x' to EXIT
#since serDataq.readline waits until a line is received, low sample rate makes
#  the program slow in response to key stroke

#get the com port from device manger and enter it here
CONST_SER_PORT = '/dev/cu.usbmodem14201'

serDataq = serial.Serial(CONST_SER_PORT)

# for Windows 10, one may run into the exception of "serial.serialutil.SerialException:
#   Cannot configure port, something went wrong. 
#   Original message: WindowsError(87, 'Incorrect Parameter.') exception."
#
# Work around until pyserial has fix:, find serialwin32.py inside the folder of
#   C:\users\??? window 10\AppData\Roaming\Python\Python37\site-packages\serial\
...
while True:
    try:
        if keyboard.is_pressed('x'):    
            #if key 'x' is pressed, stop the scanning and terminate the program
